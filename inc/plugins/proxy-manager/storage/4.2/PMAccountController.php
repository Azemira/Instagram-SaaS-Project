<?php
require_once PLUGINS_PATH . "/proxy-manager/models/ProxiesModel.php";
...
use Plugins\ProxyManager\AccountModel as PMAccountModel;
use Plugins\ProxyManager\LogModel as PMLogModel;
use Plugins\ProxyManager\ProxyModel as PMProxyModel;
use Plugins\ProxyManager\ProxiesModel as PMProxiesModel;

/**
 * Account Controller
 */
class PMAccountController extends Controller
{
    /**
     * Instagram user name
     * @var string
    ...
    /**
     * Whether proxy is allocated or not
     * @var bool
     */
    ...
    public function process()
    {
        ...
        if (isset($Route->params->id)) {
            $Account = new PMAccountModel($Route->params->id);
            ...
            $Account = new PMAccountModel(); // new account model
        }

        // Set view variables
        $this->setVariable("Accounts", $Accounts)
             ->setVariable("Account", $Account)
             ->setVariable("Pages", Controller::model("GeneralData", "pages"))
    ...
    private function save()
    {
        ...
        if ($Settings->get("data.proxy")) {
            if (Input::post("proxy") && $Settings->get("data.user_proxy")) {
                ...
                // Refresh count of proxy when user want to use their own proxy
                if(!$is_new) {
                ...
                if($is_new || $Account->get("proxy_added_by_user")) {
                    $package_id = $AuthUser->get("package_id");
                    $Proxies = new PMProxiesModel();
                    $this->proxy = $Proxies::getBestProxy($countries, $package_id);
                    $this->is_allocated = true;
                } else {
                    $this->proxy = $Account->get("proxy");
                }
        ...
        // Get profile picture URL from Instagram and download profile picture to user directory
        $ig_pic_url = $login_resp->getLoggedInUser()->getProfilePicUrl();      
        $url_parts = parse_url($ig_pic_url);   
        $ext = strtolower(pathinfo($url_parts['path'], PATHINFO_EXTENSION));
        $filename = "profile-pic-".$login_resp->getLoggedInUser()->getUsername().".".$ext;
        ...
        $Account->set("user_id", $AuthUser->get("id"))
                ...
                ->set("login_required", 0)
                ->set("proxy_added_by_user", empty(Input::post("proxy")) ? 0 : 1)
                ->save();

        // Ensure proxy is allocate to the account
        if($this->is_allocated) {
            // Log
            $Proxy = new PMProxyModel($this->proxy);
            if($Proxy->isAvailable() && $Account->isAvailable()) {
                $Log = new PMLogModel;
            ...
            if($Proxy->isAvailable()) {
                Event::trigger("refresh.count.of.proxy", $Proxy->get("proxy"));
            }
        }
    ...
    protected function handleChallengeException($Instagram, $e)
    {
        ...
        try {
            ...
            if ($challenge_resp->status != "ok") {
                $this->resp->msg = __("Please go to <a href='//instagram.com' target='_blank'>Instagram</a> and set a mobile phone number for your account.");
                $this->jsonecho();
            }
