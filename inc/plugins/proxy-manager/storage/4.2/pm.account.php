...
<html lang="<?= ACTIVE_LANG ?>">
    <head>
        ...
        <title><?= $Account->isAvailable() ?  htmlchars($Account->get("username")) : __("Add New Account") ?></title>
    </head>
    ...
    <body>
        ...
        <?php 
            $TopBar = new stdClass;
            $TopBar->title = $Account->isAvailable() ?  htmlchars($Account->get("username")) : __("Add New Account");
            $TopBar->btn = false;
            require_once(APPPATH . '/views/fragments/topbar.fragment.php');
        ?>

        <?php require_once(APPPATH . '/views/fragments/pm.account.fragment.php'); ?>
