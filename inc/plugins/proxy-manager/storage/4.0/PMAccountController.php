...
{
    /**
     * Whether proxy is allocate or not
     * @var bool
     */
    ...
    private function save()
    {
        ...
        // Check account has already added by other user or not
        $AccountModel = new PMAccountModel($username);

        if($is_new) {
            if($AccountModel->isAvailable()) {
                $this->resp->msg = __("This account has been already added by another user");
                $this->jsonecho();
            }
        } else {
        ...
        if ($Settings->get("data.proxy")) {
            if (Input::post("proxy") && $Settings->get("data.user_proxy")) {
                ...
                if($is_new || $Account->get("proxy_added_by_user")) {
                    ...
                    $proxy = $Proxies::getBestProxy($countries, $package_id);
