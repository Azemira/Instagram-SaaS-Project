...
<div id="topbar">
    <div class="clearfix">
        ...
        <?php if (!empty($TopBar->btn)): ?>
            <?php if(count($TopBar->btn) == count($TopBar->btn, COUNT_RECURSIVE)): ?>
            ...
            <?php else: ?>
                <?php foreach ($TopBar->btn as $btn): ?>
                    <a class="topbar-special-link" href="<?= !empty($btn["link"]) ? $btn["link"] : "javascript:void(0)" ?>">
        ...
        <div class="topbar-actions clearfix">
            <?php if ($AuthUser->isAdmin()): ?>
                <div class="item topbar-search">
                    <form action="https://google.co.in/search" target="_blank">
                        <a class="sli sli-magnifier icon" href="#"></a>
                        <input class="search-box"
                               type="text"
                               name="q"
                               placeholder="<?= __("Google Search") ?>"
                               autocomplete="off">
