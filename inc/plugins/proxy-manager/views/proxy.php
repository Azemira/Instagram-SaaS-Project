...
<html lang="<?= ACTIVE_LANG ?>">
    ...
    <body>
        ...
        <?php require_once(__DIR__.'/fragments/proxy.fragment.php'); ?>
        ...
        <script type="text/javascript" charset="utf-8">
            $(function(){
                ...
                $('.combobox').val('<?= $Proxy->get("country_code") ?>').trigger('change');
                <?php if(!$Proxy->isAvailable()): ?>
                var $input = $(":input[name='proxy']");
                var $refresh = $('.refresh-country');
                $input.on('paste', function() {
                    setTimeout(ProxyManager.LookUpProxy, 100);
                });
