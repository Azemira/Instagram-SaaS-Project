...
@section('title', __('Update autopilot'))

@section('content')
    <form role="form" method="post" action="{{ route('autopilot.update', $autopilot) }}" autocomplete="off">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">@lang('Update autopilot')</h3>
            </div>
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <div class="selectgroup w-100">
                        <label class="selectgroup-item">
                            <input type="radio" name="action" value="1" class="selectgroup-input" {{ $autopilot->action == '1' ? 'checked' : '' }}>
                            <span class="selectgroup-button">@lang('New Followers')</span>
                        </label>

                        <label class="selectgroup-item">
                            <input type="radio" name="action" value="2" class="selectgroup-input" {{ $autopilot->action == '2' ? 'checked' : '' }}>
                            <span class="selectgroup-button">@lang('Unfollowers')</span>
                        </label>

                        <label class="selectgroup-item">
                            <input type="radio" name="action" value="3" class="selectgroup-input" {{ $autopilot->action == '3' ? 'checked' : '' }}>
                            <span class="selectgroup-button">@lang('New Following')</span>
                        </label>

                        <label class="selectgroup-item">
                            <input type="radio" name="action" value="4" class="selectgroup-input" {{ $autopilot->action == '4' ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select name="account_id" class="form-control">
                                @foreach($accounts as $account)
                                    <option value="{{ $account->id }}" {{ $autopilot->account_id == $account->id ? 'selected' : '' }}>{{ $account->username }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label class="form-label">@lang('Name')</label>
                            <input type="text" name="name" value="{{ $autopilot->name }}" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" placeholder="@lang('Autopilot name')">
                        </div>
                    </div>
                        ...
                        <div class="row" id="date_and_time" style="display: none;">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-icon">
                                        ...
                                        <input type="text" name="starts_at" value="{{ $autopilot->starts_at }}" class="form-control {{ $errors->has('starts_at') ? 'is-invalid' : '' }} dm-date-time-picker" placeholder="@lang('Starts at')">
                                    </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-icon">
                                        <span class="input-icon-addon"><i class="fe fe-calendar"></i></span>
                                        <input type="text" name="ends_at" value="{{ $autopilot->ends_at }}" class="form-control {{ $errors->has('ends_at') ? 'is-invalid' : '' }} dm-date-time-picker" placeholder="@lang('Ends at')">
                                    </div>
                ...
                <div class="form-group" id="option_list">
                    ...
                    <select name="lists_id" class="form-control {{ $errors->has('lists_id') ? 'is-invalid' : '' }}">
                        ...
                        @foreach($lists as $list)
                            <option value="{{ $list->id }}" {{ $autopilot->lists_id == $list->id ? 'selected' : '' }}>{{ $list->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group" id="option_message" style="display: none;">
                    ...
                    <div class="emoji-picker-container">
                        <textarea rows="3" name="text" class="form-control {{ $errors->has('text') ? 'is-invalid' : '' }}" placeholder="@lang('Compose a message to be sent')" data-emojiable="true">{{ $autopilot->text }}</textarea>
                    </div>
            ...
            <div class="card-footer">
                <div class="d-flex">
                    ...
                    <button class="btn btn-success ml-auto">@lang('Update autopilot')</button>
                </div>
            </div>
        </div>
    </form>
...
@push('scripts')
    ...
    $(function() {
        ...
        @if($autopilot->lists_id)
            $('input[name="message_type"][value="list"]').trigger('click');
        @else
            $('input[name="message_type"][value="text"]').trigger('click');
        @endif

        @if($autopilot->starts_at || $autopilot->starts_at)
